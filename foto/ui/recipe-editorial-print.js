...
function escapeHtml(value){
    ...
    function collectPreparationHtml(section){
      ...
      Array.from(list.children).forEach(function(li, idx){
        ...
        // R18-037: la paginazione manuale può distribuire i <li> su più <ol>.
        // Il valore esplicito mantiene la numerazione naturale anche a cambio pagina.
        clone.setAttribute('value', String(idx + 1));
        out.appendChild(clone);
      });
      return '<section class="print-section print-body">' + out.outerHTML + '</section>'; // R18-037: numerazione naturale preservata con value sui li
    }

    function collectRecipeMeta(){
      var out = {};
      function cleanLabel(value){
        return String(value || '').replace(/[:：]\s*$/,'').replace(/\s+/g,' ').trim().toLowerCase();
      }
      var cards = document.querySelectorAll('.meta-grid .meta-card');
      cards.forEach(function(card){
        var label = cleanLabel(card.querySelector('strong') && card.querySelector('strong').textContent);
        var value = (card.querySelector('span') && card.querySelector('span').textContent || '').replace(/\s+/g,' ').trim();
        if(label && value) out[label] = value;
      });
      // R18-052: gli allergeni sono aggiunti nella quick-meta via JS
      // sincronizzato da search-data.json; la stampa editoriale non li vedeva
      // perché leggeva solo la meta-grid statica. Si legge anche la quick-meta,
      // senza modificare HTML ricetta o logica di ricerca.
      document.querySelectorAll('.recipe-quick-meta span').forEach(function(item){
        if(!item || (item.classList && item.classList.contains('recipe-quick-meta-sep'))) return;
        var strong = item.querySelector('strong');
        var label = cleanLabel(strong && strong.textContent);
        if(!label) return;
        var raw = (item.textContent || '').replace(/\s+/g,' ').trim();
        var strongText = (strong && strong.textContent || '').replace(/\s+/g,' ').trim();
        var value = raw;
        if(strongText && raw.indexOf(strongText) === 0){
          value = raw.slice(strongText.length).replace(/^[:：]?\s*/, '').trim();
        }
        if(label && value) out[label] = value;
      });
      return out;
    ...
    function collectIngredientsHtml(section){
      ...
      var table = section.querySelector('table.recipe-table') || section.querySelector('table.ingredients-table') || section.querySelector('table');
      if(!table){
        return '';
      }
      var rows = [];
      ...
      table.querySelectorAll('tbody tr, tr').forEach(function(tr){
        ...
        var isSection = tr.classList && tr.classList.contains('ingredient-section-row');
        if(!isSection && cells.length === 1 && (cells[0].getAttribute('colspan') || '') !== '') isSection = true;
        if(isSection){
          var sectionLabel = (cells[0] && cells[0].textContent || '').replace(/\s+/g,' ').trim();
          if(sectionLabel){
            rows.push('<div class="print-ingredient-section">' + escapeHtml(sectionLabel) + '</div>');
          }
          return;
        }
        ...
        line += '<span class="print-ingredient-name">' + escapeHtml(ing) + '</span>';
        if(amount){
          line += '<span class="print-ingredient-detail">' + escapeHtml(amount) + '</span>';
        }
        if(note){
      ...
      if(meta['tempo totale']) timeRows.push(['Totale', meta['tempo totale']]);
      if(meta['allergeni']) timeRows.push(['Allergeni', meta['allergeni']]);
      var ingredientAttr = escapeHtml(ingredientNames.join('|'));
      var isLong = rows.length > 14;
      var initialCols = rows.length > 32 ? 'print-kitchen-card-cols-3' : (isLong ? 'print-kitchen-card-cols-2' : 'print-kitchen-card-side');
      var cardClass = 'print-kitchen-card print-primary-block ' + (isLong ? 'print-kitchen-card-long ' : '') + initialCols;
      var html = '<section class="' + cardClass + '" data-print-occupied="1" data-ingredient-names="' + ingredientAttr + '" data-print-kitchen-card="1">';
      html += '<div class="print-ingredients-box" data-print-occupied="1"><h3>Ingredienti</h3><div class="print-ingredients-list">' + rows.join('') + '</div>';
      if(timeRows.length){
        html += '<div class="print-ingredients-meta">';
        timeRows.forEach(function(row, idx){
          if(idx) html += '<span class="print-meta-separator"> | </span>';
          html += '<span><strong>' + escapeHtml(row[0]) + ':</strong> ' + escapeHtml(row[1]) + '</span>';
        });
        html += '</div>';
      }
      html += '</div>';
      html += '<aside class="print-ingredients-media" aria-hidden="true"></aside>';
      html += '</section>';
      return html;
    }
    ...
    function buildPrintDocument(){
      ...
      // R18-029: riconoscimento sezioni robusto rispetto al DOM modificato
      // dall'overlay Ingredienti. La fonte primaria sono le classi stabili;
      // il riconoscimento per titolo resta solo come fallback retrocompatibile.
      var ingredientsSection = root.querySelector('section.ingredients-block') || null;
      var prepSection = root.querySelector('section.preparation-block') || null;
      if(!ingredientsSection || !prepSection){
        root.querySelectorAll('section.block, section').forEach(function(sec){
          if(ingredientsSection && prepSection) return;
          var h2 = sec.querySelector(':scope > h2') ||
                   sec.querySelector(':scope > .recipe-ingredients-head > h2') ||
                   sec.querySelector(':scope > .recipe-preparation-head > h2');
      ...
      html += '<style>' + [
        ...
        '.print-page{position:relative;width:210mm;min-height:265mm;margin:0 auto;padding:0 22mm;box-sizing:border-box;break-inside:avoid;page-break-inside:avoid;break-after:auto;page-break-after:auto;}',
        ...
        '.print-title{font-family:Arial,Helvetica,sans-serif;font-size:23.5pt;line-height:1.05;font-weight:700;letter-spacing:-.01em;color:#111;margin:0 0 7.5mm 0;}',
        ...
        '.print-kitchen-card-cols-1{width:54mm;max-width:54mm;margin-left:0;margin-right:auto;} .print-kitchen-card-cols-2{width:110mm;max-width:110mm;margin-left:0;margin-right:auto;} .print-kitchen-card-cols-3{width:166mm;max-width:166mm;margin-left:auto;margin-right:auto;}',
        '.print-kitchen-card-cols-1 .print-ingredients-box,.print-kitchen-card-cols-2 .print-ingredients-box,.print-kitchen-card-cols-3 .print-ingredients-box{float:none;width:100%;min-width:0;max-width:100%;margin:0;}',
        ...
        '.print-steps{margin:0;padding-left:0;list-style-position:inside;} .print-steps > li{margin:0 0 4.8mm 0;padding:0;break-inside:avoid !important;page-break-inside:avoid !important;} .print-steps > li::after{content:"";display:block;clear:both;} .print-steps > li > *:first-child{display:inline;margin-top:0;} .print-steps > li > p:first-child strong{font-weight:650;font-size:11.45pt;letter-spacing:-.005em;}',
        '.print-steps > li > p,.print-steps > li > div,.print-steps > li > ul,.print-steps > li > ol,.print-steps > li > blockquote{break-inside:avoid !important;page-break-inside:avoid !important;}',
        '.print-steps li .image-row{display:flex !important;flex-wrap:wrap !important;gap:3mm !important;align-items:flex-start !important;justify-content:center !important;clear:both !important;width:100% !important;margin:1.5mm auto 4mm auto !important;}',
        '.print-steps li .image-row > figure{float:none !important;display:block !important;flex:0 0 39mm !important;max-width:39mm !important;width:39mm !important;margin:0 !important;clear:none !important;}',
        '.print-steps li .image-row > figure img{display:block;width:100% !important;max-width:100% !important;height:auto !important;}',
        '.print-steps li .image-row > figure figcaption{font-family:Arial,Helvetica,sans-serif;font-size:7.8pt;line-height:1.25;color:#555;margin-top:1.2mm;}',
        ...
        '.print-section::after,.print-notes-block::after{content:"";display:block;clear:both;} .print-steps > li > p{margin:0 0 3.55mm 0;} .print-steps > li strong{font-weight:600;}',
    ...
    function paginatePrintDocument(doc){
      ...
      function cleanupEmptyPrintBodies(pageInfo){
        ...
        Array.prototype.slice.call(pageInfo.content.querySelectorAll('.print-body')).forEach(function(body){
          if(!body.querySelector('.print-steps li')) body.remove();
        });
      }
      ...
      function nodeHasMeaningfulContent(node){
        ...
        if(node.matches && node.matches('.print-footer, .print-notes-block, .print-kitchen-card, .print-lead, .print-section')){
          return true;
        }
        if(node.querySelector && node.querySelector('figure, img, .print-footer, .print-notes-block, .print-kitchen-card, .print-steps li')){
          return true;
        }
      ...
      function isKitchenCard(block){
        return !!(block && block.classList && block.classList.contains('print-kitchen-card'));
      }

      function setKitchenCardMode(block, mode){
        if(!isKitchenCard(block)) return;
        block.classList.remove('print-kitchen-card-side','print-kitchen-card-cols-1','print-kitchen-card-cols-2','print-kitchen-card-cols-3');
        if(mode === 'side') block.classList.add('print-kitchen-card-side');
        else if(mode === 'cols2') block.classList.add('print-kitchen-card-cols-2');
        else if(mode === 'cols3') block.classList.add('print-kitchen-card-cols-3');
        else block.classList.add('print-kitchen-card-cols-1');
      }

      function kitchenCardModes(block){
        if(!isKitchenCard(block)) return [];
        // R18-042: il box piccolo resta laterale se entra.
        // Se però il laterale non entra nello spazio residuo, si provano davvero
        // le versioni a 2 e 3 colonne prima di spostarlo alla pagina successiva.
        if(block.classList.contains('print-kitchen-card-side')) return ['side','cols2','cols3'];
        // I box lunghi usano solo layout realmente multicolonna, evitando il
        // caso largo a una colonna che crea molto bianco interno.
        return ['cols2','cols3'];
      }

      function fitKitchenCardOnPage(block, pageInfo){
        if(!isKitchenCard(block) || !pageInfo) return false;
        var modes = kitchenCardModes(block);
        for(var i=0; i<modes.length; i+=1){
          setKitchenCardMode(block, modes[i]);
          if(contentHeight(pageInfo) <= pageHeightPx + 0.5) return true;
        }
        setKitchenCardMode(block, modes[modes.length - 1] || 'cols3');
        return contentHeight(pageInfo) <= pageHeightPx + 0.5;
      }

      function appendBlockToCurrent(block){
        current.content.appendChild(block);
        if(isKitchenCard(block) && contentHeight(current) > pageHeightPx + 0.5){
          if(fitKitchenCardOnPage(block, current)) return;
          if(pageHasContent(current) && current.content.children.length > 1){
            block.remove();
            current = newPage();
            printDoc.appendChild(current.page);
            pages.push(current);
            current.content.appendChild(block);
            fitKitchenCardOnPage(block, current);
            return;
          }
        }
      ...
      originalChildren.forEach(function(child){
        ...
        if(child.classList && child.classList.contains('print-steps')){
          ...
          items.forEach(function(li){
            ...
            if(contentHeight(current) > pageHeightPx + 0.5 && pageHasContent(current) && (currentList.children.length > 1 || current.content.children.length > 1)){
              li.remove();
        ...
        if(child.classList && child.classList.contains('print-body')){
          var embeddedList = child.querySelector(':scope > .print-steps');
          if(embeddedList){
            var items = Array.prototype.slice.call(embeddedList.children).filter(function(li){
              return li && li.tagName && li.tagName.toLowerCase() === 'li';
            });
            items.forEach(function(li){
              ensurePageContentBlocks(current);
...
function injectPrintIngredientDecor(doc){
  ...
  function buildPhysicalPageInfos(){
    ...
    pages.forEach(function(page, domPageIndex){
      ...
      var topPx = Math.max(0, pageRect.top - printRect.top);
      // R18-041: le decorazioni devono cercare spazi sull'intera pagina fisica,
      // non solo nel bounding box del contenuto già presente. Altrimenti le
      // ampie aree bianche sotto un blocco corto (es. Ingredienti) non vengono
      // viste come candidabili.
      var bottomPx = topPx + nominalPageHeight;
      infos.push({
  ...
  function collectPageBlockExclusions(pageInfo){
    ...
    var sliceTop = pageInfo.topPx;
    var sliceBottom = pageInfo.bottomPx;
    return Array.prototype.slice.call(pageInfo.page.querySelectorAll('.print-kitchen-card, .print-ingredients-box, [data-print-occupied="1"], .print-notes-block, .print-footer')).map(function(node){
      // R18-042: nei box Ingredienti laterali il contenitore .print-kitchen-card
      // è largo 100% anche se il riquadro reale è solo a sinistra; escluderlo
      // dalla bitmap blocca ingiustamente tutta l'area libera a destra.
      // Si considera occupato il vero .print-ingredients-box, non il wrapper.
      if(node && node.classList && node.classList.contains('print-kitchen-card') && node.classList.contains('print-kitchen-card-side')) return null;
      var r = node.getBoundingClientRect();
      var isIngredient = !!(node && node.classList && (node.classList.contains('print-kitchen-card') || node.classList.contains('print-ingredients-box')));
  ...
  return Promise.all([
    ...
    function runEditorialPrint(){
      return new Promise(function(resolve){
        ...
        if(!html){ resolve({status:'no-print-document'}); return; }
      ...
      setTimeout(function(){ cleanup(); resolve({status:'queued'}); }, 5000);
      setTimeout(function(){ resolve({status:'queued'}); }, 700);
      });
    }
